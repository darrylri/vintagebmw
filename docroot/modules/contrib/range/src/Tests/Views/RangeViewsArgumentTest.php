...
/**
 * Tests the Drupal\range\Plugin\views\argument\Range handler.
 *
 * @group range
 */
class RangeViewsArgumentTest extends RangeViewsTestBase {
  ...
  public function testRangeViewsArgument() {
    ...
    $view->initHandlers();
    $view->argument[$this->field_name]->options['operator'] = 'within';
    $view->argument[$this->field_name]->options['include_endpoints'] = FALSE;
    ...
    $view->initHandlers();
    $view->argument[$this->field_name]->options['operator'] = 'within';
    $view->argument[$this->field_name]->options['include_endpoints'] = TRUE;
    $view->setDisplay('default');
    $this->executeView($view, [5]);
    $expected_result = [
      ['nid' => $this->nodes[0]->id()],
    ...
    $view->initHandlers();
    $view->argument[$this->field_name]->options['operator'] = 'not within';
    $view->argument[$this->field_name]->options['include_endpoints'] = FALSE;
    $view->setDisplay('default');
    $this->executeView($view, [5]);
    $expected_result = [
